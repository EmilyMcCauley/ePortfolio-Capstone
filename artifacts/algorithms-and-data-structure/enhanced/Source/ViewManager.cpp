...
float ViewManager::mouseSensitivity = 0.1f;
float ViewManager::cameraSpeedFactor = 0.1f;
...
void ViewManager::Scroll_Callback(GLFWwindow* window, double xoffset, double yoffset)
{
    ...
    if (yoffset > 0)
    {
        ViewManager::cameraSpeedFactor += 0.1f; // Zoom in or increase speed
    }
    else if (yoffset < 0)
    {
        ViewManager::cameraSpeedFactor = std::max(0.1f, ViewManager::cameraSpeedFactor - 0.1f); // Zoom out or decrease speed but not below 0.1
    }
    if (g_pCamera)
    {
        g_pCamera->MovementSpeed = 20.0f * ViewManager::cameraSpeedFactor; // Adjust the camera movement speed
    }
}
...
void ViewManager::ProcessKeyboardEvents()
{
    ...
    {
        currentProjection = ProjectionType::Perspective; // Set to perspective
    }
    if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS)
    {
        currentProjection = ProjectionType::Orthographic; // Set to orthographic
    }
...
{
    // calculate the time between the current and last frame
    float currentFrame = glfwGetTime();
    gDeltaTime = currentFrame - gLastFrame;
    gLastFrame = currentFrame;

    // handle camera movement and update the scene
    ProcessKeyboardEvents();
    ...
    // process the appropriate projection matrix based on the active projection type
 ...
 *  ToggleProjection()
 *
 *  This method is called to toggle between perspective and
 *  orthographic projections based on user input.
 ***********************************************************/
void ViewManager::ToggleProjection()
{
    if (currentProjection == ProjectionType::Perspective)
    {
        currentProjection = ProjectionType::Orthographic;
    }
    else
    {
        currentProjection = ProjectionType::Perspective;
